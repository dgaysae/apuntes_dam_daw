...
La mejor forma de entender qué es una excepción es hacer que una nos explote en la cara, así que vamos a ello. Intenta ejecutar el siguiente código:
...
Antes de ejecutarlo podríamos habernos percatado de que algo no iba a ir bien porque **una división entre 0 no es posible**. Y como no lo es, se nos muestra ese error tan raro que intuitivamente nos dice que ha habido un error al intentar hacer una operación aritmética (`ArithmeticException`).
...
Esto ocurre cuando se hace una operación que no es correcta y obtiene resultados no controlados y la JVM prefiere no seguir ejecutando el programa porque al operar con esos "datos raros" podría romperse algo importante: corromper un fichero, invadir espacios de memoria de otros procesos, etc. Algunos ejemplos pueden ser dividir un número entre cero, acceder a un objeto nulo (`null`), abrir un fichero que no existe, etc.

> [!info] 
> Veremos y comprenderemos mejor esto de las excepcioens en [[2. Excepciones|unidades posteriores]].

En este apartado veremos los conceptos básicos para:
...
try {
    ...
    /*
     Bloque que se ejecuta si en el try ocurre una excepcion01
    ...
    /*
     Bloque que se ejecuta si en el try ocurre una excepcion02
...
Hay una excepción que podemos controlar desde ya, y la explicamos en el siguiente apartado. 
