...
Una `sealed class` representa una **jerarquía de herencia cerrada y restringida**. Al marcar una clase como `sealed` (sellada), estás controlando estrictamente qué otras clases pueden heredar de ella. Todas sus subclases deben estar definidas **en el mismo paquete** (normalmente en el mismo archivo).

**La gran diferencia con un `enum`:**

- En un **`enum`**, cada constante es una _instancia única_ y todas tienen exactamente los _mismos atributos_.
- En una **`sealed class`**, sus subclases pueden ser **tanto objetos únicos (`object`) como clases normales (`class` o `data class`)**, lo que permite que cada estado contenga **datos completamente diferentes**.

---

2. Sintaxis Básica: El Estado de una API

El uso estrella de una `sealed class` en DAM es modelar el estado de una pantalla que carga datos de internet:
...
> 💡 **Nota de sintaxis:** Fíjate que todas las subclases heredan explícitamente de la clase principal usando `: ResourceState()`.

---

3. Control de Errores Absoluto con `when` (Pregunta fija de examen)

Al igual que con los enums, el compilador de Kotlin sabe exactamente cuántas subclases tiene una `sealed class`. Si utilizas `when` como expresión para evaluar una clase sellada, **el compilador te obligará a gestionar todos los casos posibles**.

Esto elimina la necesidad de usar un bloque `else` y te asegura que, si en el futuro añades un nuevo estado (ej. `NoInternet`), el código no compilará hasta que lo gestiones.
...
_Nota: Usamos la palabra clave **`is`** porque estamos comprobando el **tipo** de la clase (si es de tipo `Success`, `Error`, etc.), a diferencia de los enums donde comprobábamos el valor directamente._

---

4. `sealed class` vs `sealed interface`

En las versiones modernas de Kotlin, también existen las **`sealed interface`**. Funcionan exactamente igual que las clases selladas, pero con las ventajas de las interfaces:
...
5. Tabla Resumen para Estudiar: `enum` vs `sealed`

|Criterio|`enum class`|`sealed class`|
|---|---|---|
|**Tipo de elementos**|Constantes/Instancias únicas de un mismo tipo.|Subclases completas (`class`, `data class`, `object`).|
|**Estructura de Datos**|Todos los valores comparten los mismos campos.|Cada subclase puede tener variables totalmente distintas.|
|**Casos de uso**|Conjuntos de datos fijos (Días, Meses, Roles, Géneros).|Estados complejos, respuestas de red, eventos de UI.|
|**Uso con `when`**|Exhaustivo (obliga a cubrir todos los casos).|Exhaustivo (obliga a cubrir todos los casos con `is`).|

---

6. Aplicación real en Android (Arquitectura MVVM)

Si en 2º de DAM estáis usando **Jetpack Compose** o **XML con LiveData/StateFlow**, verás las clases selladas todos los días en tus _ViewModels_ para gestionar la UI de forma limpia:
